...
Unlike the [supervisor pattern](/oss/python/langchain/supervisor) where a supervisor routes to specialized sub-agents, or the [handoffs pattern](/oss/python/langchain/customer-support-handoffs) where agents pass control sequentially, progressive disclosure uses a **single agent** that dynamically loads specialized knowledge on demand.
