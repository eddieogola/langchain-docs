...
flowchart TD
    Start([💬 User Query]) --> SystemPrompt[📋 Skill descriptions<br/>in system prompt]

    SystemPrompt --> Decide{🤔 Which skill<br/>is relevant?}

    Decide --> LoadSkill[🔧 Call load_skill<br/>tool]

    LoadSkill --> Enriched[✨ Skill content returned<br/>in tool result]

    Enriched --> Solve[💡 Apply skill<br/>to answer]

    Solve --> Response([✅ Response])
...
<Note>
This guide demonstrates creating custom middleware. For a comprehensive guide on middleware concepts and patterns, see the [custom middleware documentation](/oss/python/python/langchain/middleware/custom).
</Note>
