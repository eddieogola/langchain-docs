...
import com.langchain.smith.models.datasets.DatasetCreate;
...
public class DatasetManagement {
    public static void main(String[] args) {
        ...
        List<Map<String, Map<String, String>>> examples = Arrays.asList(
            Map.of(
                "inputs", Map.of("question", "What is the largest mammal?"),
                "outputs", Map.of("answer", "The blue whale"),
                "metadata", Map.of("source", "Wikipedia")
            ),
            Map.of(
                "inputs", Map.of("question", "What do mammals and birds have in common?"),
                "outputs", Map.of("answer", "They are both warm-blooded"),
                "metadata", Map.of("source", "Wikipedia")
            ),
            Map.of(
                "inputs", Map.of("question", "What are reptiles known for?"),
                "outputs", Map.of("answer", "Having scales"),
                "metadata", Map.of("source", "Wikipedia")
            ),
            Map.of(
                "inputs", Map.of("question", "What's the main characteristic of amphibians?"),
                "outputs", Map.of("answer", "They live both in water and on land"),
                "metadata", Map.of("source", "Wikipedia")
            )
        );
        ...
        // Create dataset
        var dataset = client.datasets().create(DatasetCreate.builder()
            .name(datasetName)
            .description("Questions and answers about animal phylogenetics.")
            .build());
        ...
        for (var example : examples) {
            client.examples().create(ExampleCreateParams.builder()
                .datasetId(dataset.id())
                .inputs(JsonValue.from(example.get("inputs")))
                .outputs(JsonValue.from(example.get("outputs")))
                .metadata(JsonValue.from(example.get("metadata")))
                .build());
        }

        System.out.println("Dataset created with ID: " + dataset.id());
    }
}
```
...
public class DatasetFromTraces {
    public static void main(String[] args) {
        LangsmithClient client = LangsmithOkHttpClient.fromEnv();
        String datasetName = "Example Dataset";

        // Filter runs to add to the dataset
        List<Run> runs = StreamSupport.stream(
            client.runs().list("my_project").spliterator(), false)
            .filter(run -> run.parentRunId().isEmpty()) // is_root=true
            .filter(run -> run.error().isEmpty()) // error=false
        ...
        for (Run run : runs) {
            client.examples().create(ExampleCreateParams.builder()
                .datasetId(dataset.id())
                .inputs(JsonValue.from(run.inputs()))
                .outputs(JsonValue.from(run.outputs().orElse(null)))
                .build());
        }
    }
}
...
public class DatasetFromCsv {
    public static void main(String[] args) throws Exception {
        ...
        List<String> inputKeys = Arrays.asList("column1", "column2"); // replace with your input column names
        List<String> outputKeys = Arrays.asList("output1", "output2"); // replace with your output column names

        // Create dataset
        var dataset = client.datasets().create(DatasetCreate.builder()
            .name("My CSV Dataset")
            .description("Dataset created from a CSV file")
            .dataType(DatasetCreate.DataType.KV)
            .build());

        // Parse CSV and create examples
        try (BufferedReader br = new BufferedReader(new FileReader(csvFile))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                for (int i = 0; i < headers.length; i++) {
                    if (inputKeys.contains(headers[i])) {
                        inputs.put(headers[i], values[i]);
                    } else if (outputKeys.contains(headers[i])) {
                        outputs.put(headers[i], values[i]);
                    }
                }
...
```java Java
import com.langchain.smith.LangsmithClient;
import com.langchain.smith.LangsmithOkHttpClient;
...
```java Java
Iterable<Dataset> datasets = client.datasets().list();
```
...
```java Java
Iterable<Example> examples = client.examples().list(
    ExamplesListParams.builder()
        .datasetId("c9ace0d8-a82c-4b6c-13d2-83401d68e9ab")
        .build()
);
```
...
```java Java
List<String> exampleIds = Arrays.asList(
    "734fc6a0-c187-4266-9721-90b7a025751a",
    "d6b4c1b9-6160-4d63-9b61-b034c585074f",
    "4d31df4e-f9c3-4a6e-8b6c-65701c2fed13"
);
...
```java Java
Map<String, Object> inputs = new HashMap<>();
inputs.put("input", "updated input");

Map<String, Object> outputs = new HashMap<>();
outputs.put("output", "updated output");
...
client.examples().update(
    ...
    ExampleUpdateParams.builder()
        .inputs(JsonValue.from(inputs))
        .outputs(JsonValue.from(outputs))
        .metadata(JsonValue.from(metadata))
        .split(Arrays.asList("train"))
        .build()
);
```
...
inputs1.put("input", "updated input 1");

Map<String, Object> outputs1 = new HashMap<>();
outputs1.put("output", "updated output 1");

Map<String, Object> metadata1 = new HashMap<>();
metadata1.put("foo", "baz");

Map<String, Object> inputs2 = new HashMap<>();
inputs2.put("input", "updated input 2");

Map<String, Object> outputs2 = new HashMap<>();
outputs2.put("output", "updated output 2");

Map<String, Object> metadata2 = new HashMap<>();
metadata2.put("foo", "qux");

client.examples().bulkUpdate(
    ExampleBulkUpdateParams.builder()
        .updates(Arrays.asList(
            ExampleUpdate.builder()
                .exampleId(example.id())
                .inputs(JsonValue.from(inputs1))
                .outputs(JsonValue.from(outputs1))
                .metadata(JsonValue.from(metadata1))
                .split(Arrays.asList("training", "foo"))
                .build(),
            ExampleUpdate.builder()
                .exampleId(example2.id())
                .inputs(JsonValue.from(inputs2))
                .outputs(JsonValue.from(outputs2))
                .metadata(JsonValue.from(metadata2))
                .split(Arrays.asList("training"))
                .build()
        ))
        .build()
);
