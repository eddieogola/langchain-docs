...
You can also tag versions of your dataset using the SDK. Here's an example of how to tag a version of a dataset using the SDK:
...
public class DatasetVersioning {
    public static void main(String[] args) {
        LangsmithClient client = LangsmithOkHttpClient.fromEnv();

        // List datasets to find your dataset
        var datasets = client.datasets().list(DatasetListParams.builder()
            .datasetName("Toxic Queries")
            .build());

        Dataset dataset = datasets.datasets().get(0);
        System.out.println("Dataset ID: " + dataset.id());
        System.out.println("Dataset name: " + dataset.name());

        // Note: Dataset versioning and tagging are primarily managed through the UI
        // The Java SDK provides access to dataset metadata and examples
    }
}
```
...
<Note>
**Java SDK**: The Java examples in this section demonstrate how to fetch dataset examples using the same filtering capabilities as Python and TypeScript. However, the Java SDK does not include the high-level `evaluate()` function. Instead, you can fetch examples with `client.examples().list()` and implement your own evaluation logic.
</Note>
...
import com.langchain.smith.models.examples.ExamplesListParams;

public class EvaluateOnVersion {
    public static void main(String[] args) {
        LangsmithClient client = LangsmithOkHttpClient.fromEnv();

        // Fetch examples from a specific version
        Iterable<Example> examples = client.examples().list(
            ExamplesListParams.builder()
                .datasetName("Toxic Queries")
                .asOf("latest")  // specify version here
                .build()
        );
        ...
        for (Example example : examples) {
            ...
            System.out.println("Example: " + example.inputs());
        }
    }
}
```
...
public class EvaluateFiltered {
    public static void main(String[] args) {
        LangsmithClient client = LangsmithOkHttpClient.fromEnv();

        // Fetch examples with specific metadata
        Map<String, Object> metadata = new HashMap<>();
        metadata.put("desired_key", "desired_value");
        ...
        // Process filtered examples for evaluation
        for (Example example : examples) {
            // Your evaluation logic here
            System.out.println("Example: " + example.inputs());
        }
...
public class EvaluateOnSplit {
    public static void main(String[] args) {
        LangsmithClient client = LangsmithOkHttpClient.fromEnv();

        // Fetch examples from specific splits
        Iterable<Example> examples = client.examples().list(
            ExamplesListParams.builder()
                .datasetName("Toxic Queries")
                .splits(Arrays.asList("test", "training"))
                .build()
        );

        // Process examples from splits for evaluation
        for (Example example : examples) {
            // Your evaluation logic here
            System.out.println("Example: " + example.inputs());
        }
