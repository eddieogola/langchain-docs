...
1. In your terminal, prepare your environment:
    ...
    mkdir ls-prompt-quickstart-java && cd ls-prompt-quickstart-java
    mvn archetype:generate -DgroupId=com.example -DartifactId=prompt-quickstart \
      -DarchetypeArtifactId=maven-archetype-quickstart -DinteractiveMode=false
    cd prompt-quickstart
    ...
    # Then add these dependencies to your build.gradle.kts dependencies block:

    implementation("com.langchain.smith:langsmith-java:0.1.0-alpha.17")
    implementation("com.openai:openai-java:0.8.2")
    ```
    ...
    public class CreatePrompt {
        public static void main(String[] args) {
            ...
            Repo repo = client.repos().create(RepoCreate.builder()
                .repoHandle("prompt-quickstart")
                .description("A quickstart prompt")
                .isPublic(false)
                .build());

            // Define the prompt manifest
            Map<String, Object> promptManifest = Map.of(
                "messages", List.of(
                    Map.of("role", "system", "content", "You are a helpful chatbot."),
                    Map.of("role", "user", "content", "{question}")
                ),
                "input_variables", List.of("question")
            );
            ...
            System.out.println("Prompt created: " + repo.url());
        }
    }
    ```
...
1. Add the following to a `test_prompt` file:
    ...
    public class TestPrompt {
        public static void main(String[] args) {
            ...
            OpenAIClient oaiClient = OpenAIOkHttpClient.fromEnv();

            // Pull the prompt by name
            Commit commit = lsClient.commits().get("prompt-quickstart", "latest");
            Map<String, Object> manifest = commit.manifest();

            // Extract the prompt template
            @SuppressWarnings("unchecked")
            List<Map<String, String>> messages =
                (List<Map<String, String>>) manifest.get("messages");

            // Format the prompt with the question
            String question = "What is the color of the sky?";
            ChatCompletionCreateParams.Builder paramsBuilder =
                ChatCompletionCreateParams.builder().model(ChatModel.GPT_4O);

            for (Map<String, String> message : messages) {
                String role = message.get("role");
                String content = message.get("content").replace("{question}", question);
            ...
            ChatCompletion response = oaiClient.chat().completions().create(paramsBuilder.build());
...
1. To create a new version of a prompt, call the same push method you used initially with the same prompt name and your updated template. LangSmith will record it as a new commit and preserve prior versions.
    ...
    public class IteratePrompt {
        public static void main(String[] args) {
            LangsmithClient client = LangsmithOkHttpClient.fromEnv();

            // Update the prompt with new instructions
            Map<String, Object> updatedManifest = Map.of(
                "messages", List.of(
                    Map.of("role", "system", "content", "You are a helpful chatbot. Speak in Spanish."),
                    Map.of("role", "user", "content", "{question}")
                ),
                "input_variables", List.of("question")
            );

            // Push the updated prompt (creates a new commit)
            client.commits().update(
                "prompt-quickstart",
                CommitUpdateParams.builder()
                    .manifest(JsonValue.from(updatedManifest))
                    .build()
            );
