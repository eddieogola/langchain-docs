...
var projectRuns = client.runs().query(RunQueryParams.builder()
    .addSession("<your_project>")
    .build());

for (var run : projectRuns.runs()) {
    System.out.println("Run ID: " + run.id() + ", Name: " + run.name());
}
```
...
var todaysLlmRuns = client.runs().query(RunQueryParams.builder()
    .addSession("<your_project>")
    .startTime(Instant.now().minus(1, ChronoUnit.DAYS))
    .runType("llm")
    .build());

for (var run : todaysLlmRuns.runs()) {
    System.out.println("LLM Run: " + run.name());
}
```
...
```java Java
var rootRuns = client.runs().query(RunQueryParams.builder()
    .addSession("<your_project>")
    .isRoot(true)
    .build());

for (var run : rootRuns.runs()) {
    System.out.println("Root Run: " + run.name());
}
```
...
```java Java
var correctRuns = client.runs().query(RunQueryParams.builder()
    .addSession("<your_project>")
    .filter("eq(error, null)")
    .build());

for (var run : correctRuns.runs()) {
    System.out.println("Successful Run: " + run.name());
}
```
...
var selectedRuns = client.runs().query(RunQueryParams.builder()
    .id(runIds)
    .build());

for (var run : selectedRuns.runs()) {
    System.out.println("Run: " + run.id() + " - " + run.name());
}
```
...
var threadRuns = client.runs().query(RunQueryParams.builder()
    .addSession("<your_project>")
    .filter(filterString)
    .isRoot(true)
    .build());

for (var run : threadRuns.runs()) {
    System.out.println("Thread Run: " + run.name());
}
```
...
```java Java
var runs = client.runs().query(RunQueryParams.builder()
    .addSession("<your_project>")
    .filter("eq(name, \"extractor\")")
    .traceFilter("and(eq(feedback_key, \"user_score\"), eq(feedback_score, 1))")
    .build());
...
// Check for runs with user_id=4070f233-f61e-44eb-bff1-da3c163895a3
var specificRuns = client.runs().query(RunQueryParams.builder()
    .addSession("default")
    .filter("and(eq(metadata_key, 'user_id'), eq(metadata_value, '4070f233-f61e-44eb-bff1-da3c163895a3'))")
    .build());
```
...
```java Java
// Find all runs where the metadata does not contain a "conversation_id" key
var runs1 = client.runs().query(RunQueryParams.builder()
    .addSession("default")
    .filter("and(neq(metadata_key, 'conversation_id'))")
    .build());

// Find all runs where the conversation_id in metadata is not "a1b2c3d4-e5f6-7890"
var runs2 = client.runs().query(RunQueryParams.builder()
    .addSession("default")
    .filter("and(eq(metadata_key, 'conversation_id'), neq(metadata_value, 'a1b2c3d4-e5f6-7890'))")
    .build());

// Find all runs where there is no "conversation_id" metadata key and the "a1b2c3d4-e5f6-7890" value is not present
var runs3 = client.runs().query(RunQueryParams.builder()
    .addSession("default")
    .filter("and(neq(metadata_key, 'conversation_id'), neq(metadata_value, 'a1b2c3d4-e5f6-7890'))")
    .build());

// Find all runs where the conversation_id metadata key is not present but the "a1b2c3d4-e5f6-7890" value is present
var runs4 = client.runs().query(RunQueryParams.builder()
    .addSession("default")
    .filter("and(neq(metadata_key, 'conversation_id'), eq(metadata_value, 'a1b2c3d4-e5f6-7890'))")
    .build());
```
