...
public class UsageMetadataExample {
    public static void main(String[] args) {
        ...
        Span llmSpan = tracer.spanBuilder("chat_model")
            .setAttribute("langsmith.span.kind", "llm")
            .setAttribute("ls_provider", "my_provider")
            .setAttribute("ls_model_name", "my_model")
            .startSpan();
        ...
        try (Scope scope = llmSpan.makeCurrent()) {
            ...
            ChatCompletion completion = client.chat().completions().create(
                ChatCompletionCreateParams.builder()
                    ...
                    .addUserMessage("I'd like to book a table for two.")
                    .build()
            );

            // Attach token usage to the span as attributes
            llmSpan.setAttribute("gen_ai.usage.input_tokens", 27);
            llmSpan.setAttribute("gen_ai.usage.output_tokens", 13);
            llmSpan.setAttribute("gen_ai.usage.total_tokens", 40);

            // Add detailed token breakdown
            llmSpan.setAttribute("gen_ai.usage.input_token_details.cache_read", 10);
...
public class UsageMetadataOutputExample {
    public static void main(String[] args) {
        // Configure OpenTelemetry
        OpenTelemetryConfig.builder()
            .fromEnv()
        ...
        try (Scope scope = llmSpan.makeCurrent()) {
            ...
            completion.usage().ifPresent(usage -> {
                ...
                llmSpan.setAttribute("gen_ai.usage.input_tokens", usage.promptTokens());
                llmSpan.setAttribute("gen_ai.usage.output_tokens", usage.completionTokens());
                llmSpan.setAttribute("gen_ai.usage.total_tokens", usage.totalTokens());
            });
...
public class DirectCostExample {
    public static void main(String[] args) {
        // Configure OpenTelemetry
        OpenTelemetryConfig.builder()
            .fromEnv()
        ...
        try (Scope scope = llmSpan.makeCurrent()) {
            ...
            // Specify cost directly (in dollars) for non-linear pricing models
            llmSpan.setAttribute("gen_ai.usage.input_cost", 1.1e-6);
            llmSpan.setAttribute("gen_ai.usage.input_cost_details.cache_read", 2.3e-7);
            llmSpan.setAttribute("gen_ai.usage.output_cost", 5.0e-6);
...
public class ToolCostExample {
    public static void main(String[] args) {
        ...
        Span toolSpan = tracer.spanBuilder("get_weather")
        ...
        try (Scope scope = toolSpan.makeCurrent()) {
            ...
            String weatherData = String.format(
                "{\"temperature_f\": 68, \"condition\": \"sunny\", \"city\": \"%s\"}",
                city
            );

            // Cost for this tool call (computed however you like)
            double toolCost = 0.0015;

            // Attach usage metadata to the LangSmith span
            toolSpan.setAttribute("gen_ai.usage.total_cost", toolCost);
...
public class CostTrackingExample {
    public static void main(String[] args) {
        ...
        // Method 1: Automatic cost tracking from token usage
        // The WrappedOpenAIClient automatically tracks token usage
        ChatCompletion completion = client.chat().completions().create(
            ChatCompletionCreateParams.builder()
                .model(ChatModel.GPT_4O_MINI)
                .addSystemMessage("You are a helpful assistant.")
        ...
        completion.usage().ifPresent(usage -> {
            ...
            // LangSmith will automatically calculate costs based on model pricing
        });

        // Method 2: Manual cost tracking for custom operations (e.g., tool calls)
        Span toolSpan = tracer.spanBuilder("get_weather")
            .setAttribute("langsmith.span.kind", "tool")
            .startSpan();
        ...
        try (Scope scope = toolSpan.makeCurrent()) {
            // Your tool logic here
            String weatherData = "{\"temperature\": 68, \"condition\": \"sunny\"}";

            // Manually track cost for this tool call
            toolSpan.setAttribute("gen_ai.usage.total_cost", 0.0015);
